...
generators = (
    Greedy=GreedyGenerator(loss=loss), 
    Generic=GenericGenerator(loss=loss),
    REVISE=REVISEGenerator(loss=loss),
    DICE=DiCEGenerator(loss=loss)
)
```
...
target = 1.0
experiment = Experiment(counterfactual_data, target, models, generators, 5)
```
