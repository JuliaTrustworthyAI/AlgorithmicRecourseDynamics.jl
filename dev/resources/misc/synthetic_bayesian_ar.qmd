...
```{julia}
using CounterfactualExplanations.Models: BayesianLogisticModel
M = BayesianLogisticModel(reshape(model.μ,1,length(model.μ)), model.Σ); # repare for usage with CounterfactualExplanations.jl
x̅ = [50.0, 60.0]; # factual
using Random
Random.seed!(1234)
y̅ = round(CounterfactualExplanations.Models.probs(M, x̅)[1])
...
```{julia}
recourse_generic = generate_recourse(generic, x̅, M, target, γ) # generate recourse
recourse_greedy = generate_recourse(greedy, x̅, M, target, γ);
```
